...
export default function Home() {
  ...
  const eventData = [
    { title: "Morning Yoga & Meditation Session", instructor: "ครูจันทรากานต์", date: "Sunday, February 2, 2024", time: "7:00 AM - 8:30 AM", location: "สวนลุมพินี กรุงเทพฯ (จุดนัดพบ: ลานตะวัน)", members: "22/30", spaces: "8", img: "https://images.unsplash.com/photo-1552674605-db6ffd4facb5?auto=format&fit=crop&w=600&q=80" },
    { title: "Tea Tasting & Mindfulness Workshop", instructor: "คุณศิริพงษ์", date: "Saturday, February 8, 2024", time: "2:00 PM - 4:00 PM", location: "ATC Community Space ชั้น 2", members: "12/15", spaces: "3", img: "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?auto=format&fit=crop&w=600&q=80" }
  ];
      ...
      <div className="w-full max-w-auto bg-[#F5F3E9] shadow-sm overflow-hidden">
                ...
                <SwiperSlide key={index}>
                  <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row text-left border border-gray-100 mx-1">
                    <div className="p-10 md:w-3/5 flex flex-col justify-center">
                      <div className="flex gap-2 mb-6">{[1,2,3].map(i=><div key={i} className="w-10 h-3 bg-[#AEBC9F] rounded-full opacity-40"></div>)}</div>
                      <h3 className="text-[24px] font-bold text-gray-800 mb-6 leading-tight">{event.title.split('&').map((text,i)=><span key={i}>{text}{i===0&&<><br/>&</>}</span>)}</h3>
                      <div className="text-[16px] text-gray-600 space-y-3 mb-10"><p>👤 {event.instructor}</p><p>📅 {event.date}</p><p>⏰ {event.time}</p><p>📍 {event.location}</p></div>
                    ...
                    <div className="md:w-2/5 p-6 pl-0 hidden md:block"><img src={event.img} alt={event.title} className="w-full h-full object-cover rounded-2xl shadow-inner" /></div>
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
        ...
        <section className="py-16 px-6 max-w-[850px] mx-auto">
          <div className="flex flex-col md:flex-row items-center gap-10 bg-white/50 p-8 rounded-3xl shadow-sm border border-white/40">
            <div className="w-full md:w-1/2 h-[250px] overflow-hidden rounded-2xl shadow-lg"><img src="https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=800&q=80" alt="Community group" className="w-full h-full object-cover" /></div>
            <div className="w-full md:w-1/2 text-left">
              <h2 className="text-[24px] font-bold text-gray-800 mb-6 leading-tight">เหล่าสาวกชามัวรออะไรอยู่ล่ะ<br/>มาร่วมกับพวกเราสิ!</h2>
          ...
          <div className="mt-16 text-center">
            ...
            <div className="flex justify-center gap-6">
              <button onClick={openRegister} className="bg-[#485B3B] text-white px-10 py-3 rounded-full font-bold shadow-lg hover:bg-[#3a4a2f] transition-all">สมัครสมาชิก</button>
              <Link to="/events"><button className="bg-white text-[#485B3B] border-2 border-[#485B3B] px-10 py-3 rounded-full font-bold hover:bg-[#485B3B] hover:text-white transition-all">ดูกิจกรรมทั้งหมด</button></Link>
            </div>
          </div>
        </section>
