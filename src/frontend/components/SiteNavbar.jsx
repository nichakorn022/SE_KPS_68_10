...
function itemClass(isActive) {
  if (isActive) {
    return "rounded-full px-4 py-2 transition-all bg-[#485B3B]/12 text-[#485B3B] font-bold";
  }
    ...
    <>
      <nav className="fixed top-0 z-50 flex w-full items-center justify-between bg-[#AEBC9F] px-8 py-2 shadow-sm">
        ...
        <div className="flex items-center gap-6 pr-4 text-[17px] font-medium text-[#4a4a4a] md:gap-12">
          ...
          <Link to="/shop" className={itemClass(active === "shop")}>
            Shop
          </Link>
          <Link to="/events" className={itemClass(active === "events")}>
            Event
          </Link>
